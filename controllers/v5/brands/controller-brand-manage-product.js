...
async  function function_export(req, res, next) {
	try {
		...
		res.send({ 
			"error" : "1", 
			"position":"web->controller->brands->manage->product",
			"message": error_send 
		}); 
		return;			
	}	
	...
	try {
		...
		datas_info = {
			'title' 				: 'Sản phẩm thương hiệu ' + data_api_resuilt[5][0].brands_name,
		...
		res.render( ojs_configs.view_version + '/brands/manage-product',  data_send );
	}
	catch(error){
			var evn = ojs_configs.evn;
			//evn = "dev";
