...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->brand->manage->show",
				"message": error_send 
			}); 
		...
		try {
			var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
					...
					'/brands/show?c1=' + brand_id + "&c2=" + store_id,
					token
				);	
		...
		try {
			...
			res.render( ojs_configs.view_version + '/brands/manage-show',  data_send );
		}
		catch(error){
			var evn = ojs_configs.evn;
			////evn = "dev";
