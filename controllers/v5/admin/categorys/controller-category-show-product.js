...
async  function function_export(req, res, next) {
	try {
		...
		return res.send({ 
			"error" : "1", 
			"position":"web->controller->categorys->show-product",
			"message": error_send 
		}); 
	...
	var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
			...
			'/admin/categorys/show-product?c1=' + category_id, 
			token
		);	
	...
	try {
		...
		datas_info = {
			'title' 			: 'Sản phẩm của danh mục ' + data_api_resuilt[3][0].category_general_speciality_name,
		...
		res.render( ojs_configs.view_version + '/categorys/general/speciality/admin-show-product', data_send );
	}
	catch(error){
			var evn = ojs_configs.evn;
			////evn = "dev";
