...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controller->admin->orders->detail->delete",
				"message": error_send 
			}); 
						
		}		
		//return res.send({"error":"00","message":[detail_id]});
		//	
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_delete(
				ojs_configs.domain + '/api/appdalacom/' + config_api.API_APPDALACOM_VERSION + '/admin/orders/detail-delete?c1='+ detail_id,
				token
			);	
