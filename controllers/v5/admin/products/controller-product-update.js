...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->products->update",
				"message": error_send 
			}); 
						
		}		
		//return res.send({"error":"00","message":[product_id,datas]});
		//	
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_put(
				ojs_configs.domain + '/api/appdalacom/' + config_api.API_APPDALACOM_VERSION + '/admin/products/update?c1='+ product_id,
				datas,
				token
			);	
