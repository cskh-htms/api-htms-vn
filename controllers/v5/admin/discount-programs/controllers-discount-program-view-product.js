...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->discount->view-product",
				"message": error_send 
			}); 
						
		}		
		//return res.send({"error":"00","message":[product_id]});
		//	
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
				...
				'/admin/discounts/view-product?c1=' + product_id,
				token
			);	
		...
		res.render( ojs_configs.view_version + '/masterpage/widget-product-view-discount', data_send );	
