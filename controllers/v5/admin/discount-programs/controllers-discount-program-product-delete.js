...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			var product_link_id = req.params.product_link_id;
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->discounts->product-delete",
				"message": error_send 
			}); 
						
		}		
		//return res.send({"error":"00","message":[product_link_id]});
		//	
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_delete(
				...
				'/admin/discounts/product-delete?c1='+ product_link_id,
				token
			);	
