...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->reviews->show",
				"message": error_send 
			}); 
						
		}	
		//return res.send([review_id]);
		//	
		...
		try {
			var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
					...
					'/admin/reviews/show?c1=' + review_id,
					token
				);	
		...
		try {
			...
			res.render( ojs_configs.view_version + '/reviews/speciality/admin-show', data_send );
		}
		catch(error){
			var evn = ojs_configs.evn;
			////evn = "dev";
