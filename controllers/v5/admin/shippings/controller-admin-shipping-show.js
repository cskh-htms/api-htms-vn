...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->shippings->show",
				"message": error_send 
			}); 
		...
		try {
			var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
					...
					'/admin/shippings/show?c1=' + shipping_id,
					token
				);	
		...
		try {
			...
			datas_info = {
				...
				'shipping_list' 	: data_api_resuilt[3],
			}
			...
			res.render( ojs_configs.view_version + '/shipping/speciality/admin-show', data_send );
		}
		catch(error){
			var evn = ojs_configs.evn;
			////evn = "dev";
