...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->shippings->show-all",
				"message": error_send 
			}); 
		...
		try {
			...
			res.render( ojs_configs.view_version + '/shipping/speciality/admin-show-all', data_send );
		}
		catch(error){
			var evn = ojs_configs.evn;
			////evn = "dev";
