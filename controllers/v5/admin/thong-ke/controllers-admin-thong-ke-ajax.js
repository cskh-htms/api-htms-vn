...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->thong-ke-ajax",
				"message": error_send 
			}); 
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_post(
				...
				config_api.API_APPDALACOM_VERSION + '/admin/thong-ke/ajax',
				datas,
				token
			);				
		...
		return  res.render( ojs_configs.view_version + '/masterpage/widget-thong-ke-admin-show-all', data_send );	
