...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->admin->thong-ke-marketing-ajax",
				"message": error_send 
			}); 
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_post(
				...
				config_api.API_APPDALACOM_VERSION + '/admin/thong-ke/marketing-ajax',
				datas,
				token
			);	
		...
		//@ send to web	
		data_send = {
			'order_list' 	: 	data_api_resuilt[1],
			'order_list2' 	: 	data_api_resuilt[2]
		}
		
		
		//return res.send( data_send );
		
		return  res.render( ojs_configs.view_version + '/masterpage/widget-thong-ke-marketing-admin-show-all', data_send );	
