...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->option->manage->show",
				"message": error_send 
			}); 
			return;			
		}	
		...
		try {
			var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
					...
					'/options/show?c1=' + option_id + "&c2=" + store_id,
					token
				);	
		...
		try {
			...
			res.render( ojs_configs.view_version + '/options/speciality/manage-show',  data_send );
		}
		catch(error){
			var evn = ojs_configs.evn;
			//evn = "dev";
