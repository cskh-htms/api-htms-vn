...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controller->option->manage->add",
				"message": error_send 
			}); 
			return;			
		}		
		...
		try {
			...
			datas_info = {
				...
				'option_list'			: data_api_resuilt[4],			
			}			
			data_send = {
				'title' 				: 'Tạo doption',
				'users_type' 			: ojs_shares_others.get_users_type(token),
			...
			//res.send(data_send);
			//return;		
			res.render( ojs_configs.view_version + '/options/speciality/manage-add',  data_send );
