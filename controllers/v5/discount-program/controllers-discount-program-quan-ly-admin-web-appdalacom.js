...
async  function discount_program_quan_ly_admin(req, res, next) {
	try {
		...
		res.send({ 
			"error" : "1", 
			"position":"controller->web-discount-program->discount_program_quan_ly_admin",
			"message": error_send 
		}); 
		return;			
	}
	...
	try {
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
				...
				'/discount-program/speciality/quan-ly-admin/',
				token
			);	
	...
	try {
		...
		datas_info = {
			'title' 				: 'Chương trình khuyến mãi cần duyệt',
...
module.exports = discount_program_quan_ly_admin;
