...
async  function function_export(req, res, next) {
	try {
		...
		res.send({ 
			"error" : "1", 
			"position":"controller->web-discount-program->discount_program_product_add_meny",
			"message": error_send 
		}); 
		return;			
	}
	...
	try {
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_get(
				...
				'/discount-program/speciality/product/add-meny?c1=' + 
	...
	try {
		...
		res.render( ojs_configs.view_version + '/discount-program/speciality/product-add-meny', data_send );	
	}
	catch(error){
			var evn = ojs_configs.evn;
			//evn = "dev";
