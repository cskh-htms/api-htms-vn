...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->product->delete",
				"message": error_send 
			}); 
						
		}		
		//return res.send({"error":"01","message":[product_id,store_id]});
		//	
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_delete(
				...
				'/products/delete?c1='+ product_id + '&c2=' + store_id,
				token
			);	
		...
		if(data_api_resuilt.error){		
			...
			return res.send({ 
				"error" : "99", 
				"position":"web->appdalacom->controllers->products->update",
				"message": error_send 
			}); 
