...
async  function function_export(req, res, next) {
	...
	try {	
		...
		try {
			...
			return res.send({ 
				"error" : "1", 
				"position":"web->appdalacom->controllers->product->update-show-hide",
				"message": error_send 
			}); 
		...
		var data_api_resuilt = await ojs_shares_fetch_data.get_data_send_token_put(
				...
				'/products/update-show-hide?c1='+ product_id,
				datas,
				token
			);	
		...
		if(data_api_resuilt.error){		
			...
			return res.send({ 
				"error" : "99", 
				"position":"web->appdalacom->controllers->products->update-stock",
				"message": error_send 
			}); 
