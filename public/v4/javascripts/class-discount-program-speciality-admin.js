...
$(document).ready(function($){
	...
	ojs_discount_program_speciality_admin = {	
		...
		no_product_to_discount: function(datas,link_id){		
			...
			//goi api
			 $.ajax({
			  ...
			  url : ojs_loader.host + "/discount-program/speciality/product-denied/" + link_id,
		...
		//@
		//@ 8.[add_product_to_discount]
		add_product_to_discount: function(datas,link_id){		
			//console.log(datas,link_id);
			//return;
			...
			//goi api
			 $.ajax({
			  type : "put",	  
			  contentType : "application/json",
			  url : ojs_loader.host + "/discount-program/speciality/product-add/" + link_id,
			  data : JSON.stringify(datas),
			  dataType : 'json',
			  beforeSend:  function(xhr){
				ojs_loadding.ajax_show_loadding();
		...
		show_content_product: function(product_id){		
			...
			//goi api
			 $.ajax({
			  type : "get",	  
			  url : ojs_loader.host + "/discount-program/speciality/view-product/" + product_id,
			  beforeSend:  function(xhr){
				ojs_loadding.ajax_show_loadding();
			  },			  
			  error: function (request, status, error) {
			 ...
			 $.ajax({
			  ...
			  success : function(result) {
					...
					if(result.error){
						...
						ojs_message.message_ok_show(" Đã tạo chương trình ","/discount-program/speciality/");
					}				
					ojs_loadding.ajax_hide_loadding();
		...
		//@
		//@ [ajax_update_admin]
		ajax_update_admin: function(datas,discount_program_id){		
		...
		//@
		//@ [ajax_delete_admin]
		ajax_delete_admin: function(discount_program_id){		
			//alert(discount_program_id);
			//return;
			//goi api
			 $.ajax({
			  type : "delete",	  
			  url : ojs_loader.host + "/discount-program/speciality/delete/" + discount_program_id,
			  beforeSend:  function(xhr){
				ojs_loadding.ajax_show_loadding();
			  },			  
			  error: function (request, status, error) {
