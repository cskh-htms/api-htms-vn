...
let sql_select_all = 	"" + 	
	...
	ojs_configs.db_prefix  + "news_user_id as news_user_id, " + 	
	"DATE_FORMAT(" + ojs_configs.db_prefix  + "news_date_created," + "'%Y/%m/%d %H:%i:%s'"  + ") as news_date_created, " + 	
	ojs_configs.db_prefix  + "news_category_id as news_category_id, " + 
	...
	ojs_configs.db_prefix  + "news_status as news_status, " + 
...
let sql_link_default = 	""  + 
	...
	ojs_configs.db_prefix + "news_user_id  = " + 
	ojs_configs.db_prefix + "users_ID "   + 

	" LEFT JOIN " + 
	ojs_configs.db_prefix + "category_news  ON  " + 
	ojs_configs.db_prefix + "news_category_id  = " + 
...
var insert_news_general = async function (datas) {
	//@
	let sql_text = "INSERT INTO " + ojs_configs.db_prefix + "news  SET ?";
	let dataGo = {
...
var check_news_link = async function (category_id) {
	//create sql text
	let sql_text = 	"SELECT " + ojs_configs.db_prefix + "news_ID " + 
					sql_from_default + 
					sql_link_default + 
					"where " + 
					ojs_configs.db_prefix + "news_category_id = '" + category_id + "' " + 
					sql_order_default
	//@
	try {
		return new Promise( (resolve,reject) => {
...
var update_news_general = async function (datas,news_id) {
	...
	let table_name  = ojs_configs.db_prefix + "news ";
	let field_where  = ojs_configs.db_prefix + "news_ID ";
	//create sql text
