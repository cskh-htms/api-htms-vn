...
let sql_order_default = " order by " + 
	ojs_configs.db_prefix + "shipping_speciality_name " ;
...
var insert_shipping_spaciality = async function (datas) {
	//@
	let sql_text = "INSERT INTO " + ojs_configs.db_prefix + "shipping_speciality  SET ?";
	let dataGo = {
...
var get_one_shipping_spaciality = async function (shipping_id) {
	...
	let sql_text = 	"SELECT " +  sql_select_all + 
					...
					sql_link_default + 
					" where " + 
 					ojs_configs.db_prefix + "shipping_speciality_ID = '" + shipping_id + "' " + 
					sql_order_default
	//@
	try {
		return new Promise( (resolve,reject) => {
...
var update_shipping_spaciality = async function (datas,shipping_id) {
	...
	let table_name  = ojs_configs.db_prefix + "shipping_speciality ";
	let field_where  = ojs_configs.db_prefix + "shipping_speciality_ID ";
	//create sql text
