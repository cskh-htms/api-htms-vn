...
async  function function_export(req, res, next) {
	try {
		...
		res.send({ "error" : "2", "position":"ctl-users->check_token app", "message": error_send } );
		return;	
	}	
	...
	try{	
				...
				token_search(token,res).then( results => {
					...
					if(Object.entries(results).length  > 0) {
						...
						//nếu đăng nhập không có user theo id token database
							...
							res.send({ "error" : "7", "position":"ctl-users->check_token app", "message": error_send } );
							return;	 		
						});		
					//@
					//@
		...
		res.send({ "error" : "10", "position":"ctl-users->check_token app", "message": error_send } );
		return;	  
	}		
