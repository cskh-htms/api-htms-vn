...
async  function function_export(req, res, next) {
	try {
		...
		if(user_id != de_token.users_ID){
			return res.send({ 
				"error" : "01", 
				"position" : "api/web/v5/ctronller/controllers-user-change-password-app-client",
		...
		return res.send({ "error" : "2", "position":"ctl-users->change-password-app-client", "message": error_send } );
