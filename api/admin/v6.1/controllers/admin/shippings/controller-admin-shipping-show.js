...
const shipping_get_one = require('../../../../../lib/' + config_api.API_LIB_VERSION + '/shippings/shipping-get-one');
const shipping_search = require('../../../../../lib/' + config_api.API_LIB_VERSION + '/shippings/shipping-search');
...
async  function function_export(req, res, next) {
	...
	try{
		...
		try {
			...
			var shipping_id = 0;
			if(req.query.c1){
				shipping_id = req.query.c1;
			}else{
				return res.send({ 
					"error" : "01", 
					"position" : "api->appdalacom->controller->admin->shipping->show",
		...
		try{	
			...
			var fn_get_shipping_taget = new Promise((resolve, reject) => {
				let result = shipping_get_one(shipping_id,res);
				resolve(result);
			});	
			promise_all.push(fn_get_shipping_taget);		
			...
			var fn_get_shipping_list = new Promise((resolve, reject) => {
				let result = shipping_search(data_shipping_list,res);
				resolve(result);
			});	
			promise_all.push(fn_get_shipping_list);		
