...
const function_export = async function(token,image_id,res){
	//return coupon_id;
	try {
	...
	try {
			...
			[
				...
				"where" :
					[
						...
						{   
							"field"     :"uploads_infomation_image_id",
							"value"     : image_id,
							"compare" : "="
						}  						
					]    
		...
		var upload_search_resuilt = await upload_search(datas,res);
		
		if(upload_search_resuilt.length >  0 && upload_search_resuilt[0].coupon_speciality_ID > 0){
			return 1;
		}else{
			return 0;
		};
