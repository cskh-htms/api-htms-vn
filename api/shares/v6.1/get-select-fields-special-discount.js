...
const select_field_special = function(field,res){
	...
	try {
		if(field == "check_expired"){	
			...
			"(CASE " + 
				"WHEN ( " + config_api.PREFIX  + "discount_program_time_type  = 0 ) THEN " +  
					" 1 " +  
					
				"WHEN ( UNIX_TIMESTAMP(" + config_api.PREFIX + "discount_program_date_end) - UNIX_TIMESTAMP() > 0 ) THEN " + 
					" 1 " +  
			...
			sql_field_check = sql_field_check + 
			"IF(" + config_api.PREFIX + "discount_program_details_limit_day = 0,-1, " + 
				"UNIX_TIMESTAMP() - (UNIX_TIMESTAMP(" + 
				config_api.PREFIX + "discount_program_details_date_created) + ( " + 
				config_api.PREFIX + "discount_program_details_limit_day * 24 * 60 * 60) )" + 
			") ";			
			return sql_field_check;
		}else{
			return " ";
