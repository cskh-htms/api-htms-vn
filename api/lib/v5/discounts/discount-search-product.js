...
const discount_search_product = async function (datas,res) {
	...
	try{	
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗi discount search product, Vui lòng liên hệ admin" 
			);
		res.send({ 
			"error" : "1",
			"position" : "discount search product", 
			"message": error_send 
			}); 
		return;	
	}	
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: get_sql_search_group, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					res.send({ 
						"error" : "2",
						"position" : "lib/discounts/discount search product", 
						"message": error_send 
					}); 
					return;
				}
...
module.exports = discount_search_product;
