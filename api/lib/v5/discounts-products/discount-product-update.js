...
const function_export = async function (datas,id,res) {
	...
	let table_name  = config_database.PREFIX + "discount_program_product_link ";
	let field_where  = config_database.PREFIX + "discount_program_product_link_ID ";
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: sql_text, timeout: 20000 } , ( err , results , fields ) => {
				if( err ) {
					...
					return res.send({ 
						"error" : "10", 
						"position" : "lib/discount-products/coupon-update",
						"message": error_send 
					}); 
