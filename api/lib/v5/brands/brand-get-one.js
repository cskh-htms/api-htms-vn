...
const fields_get = require('./brand-fields-get.js');
...
const function_export = function (id,res) {
	...
	try{	
		var get_sql_search_group = "SELECT " + 
			...
			"where " + config_database.PREFIX + "brands_ID =" + id;  
	}
	catch(error){
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗi lib->brands->get one, Vui lòng liên hệ admin" 
			);
		res.send({ 
			"error" : "1",
			"position" : "lib->brands->get one", 
			"message": error_send 
			}); 
		return;	
	}	
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: get_sql_search_group, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					res.send({ 
						"error" : "2",
						"position" : "lib/brands/store-get-one", 
						"message": error_send 
					}); 
					return;
				}
