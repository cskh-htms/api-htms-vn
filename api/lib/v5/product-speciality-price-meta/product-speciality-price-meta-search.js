...
const fields_get = require('./product-speciality-price-meta-fields-get.js');
...
const function_export = async function (datas,res) {
	...
	try{	
		...
		res.send({ 
			"error" : "1",
			"position" : "lib/discount-product/discount-product-price-meta-search", 
			"message": error_send 
			}); 
		return;	
	}	
			...
			connection.query( { sql: get_sql_search_group, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					var error_send = ojs_shares_show_errors.show_error( 
							evn, 
							err, 
							"Lỗi lib/discount-product/discount-product-price-meta-search, Vui lòng liên hệ admin" 
						);
					res.send({ 
						"error" : "2",
						"position" : "lib/discount-product/discount-product-price-meta-search", 
