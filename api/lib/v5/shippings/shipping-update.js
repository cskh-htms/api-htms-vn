...
const function_export = async function (datas,id,res) {
	...
	let table_name  = config_database.PREFIX + "shipping_speciality ";
	let field_where  = config_database.PREFIX + "shipping_speciality_ID ";
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: sql_text, timeout: 20000 } , ( err , results , fields ) => {
				if( err ) {
					...
					res.send({ 
						"error" : "10", 
						"position" : "lib/shipping_speciality/update",
						"message": error_send 
					}); 
					return;	
				}
