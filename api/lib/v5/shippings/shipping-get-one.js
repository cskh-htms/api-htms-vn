...
const fields_get = require('./shipping-fields-get.js');
...
const function_export = function (id,res) {
	...
	try{	
		var get_sql_search_group = "SELECT " + 
			...
			"where " + config_database.PREFIX + "shipping_speciality_ID =" + id;  
	}
	catch(error){
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗi lib->shipping_speciality->get one, Vui lòng liên hệ admin" 
			);
		res.send({ 
			"error" : "1",
			"position" : "lib->shipping_speciality->get one", 
			"message": error_send 
			}); 
		return;	
	}	
