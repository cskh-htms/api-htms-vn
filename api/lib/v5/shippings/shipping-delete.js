...
const function_export = function (id,res) {
	...
	var table_name  = config_database.PREFIX + "shipping_speciality ";
	var field_where  = config_database.PREFIX + "shipping_speciality_ID ";
	//create sql text
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: sql_text, timeout: 20000 }  , ( err , results , fields ) => {
				if( err ) {
					...
					res.send({ 
						"error" : "10", 
						"position" : "lib->shipping_speciality->delete",
						"message": error_send 
					}); 
					return;					
				}
