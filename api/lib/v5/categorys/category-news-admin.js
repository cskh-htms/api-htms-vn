...
const category_news_admin = async function (res) {
	...
	" where " + 
		config_database.PREFIX + "category_general_speciality_admin_status in (0,2) "  + 
	" and " + 
			...
			connection.query( { sql: sql_text, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					var error_send = ojs_shares_show_errors.show_error( 
							evn, 
							err, 
							"Lỗi category news admin, Vui lòng liên hệ admin" 
						);
					res.send({ 
						"error" : "1",
						"position" : "lib/categorys/category news admin", 
						"message": error_send 
					}); 	
					return;					
				}
...
module.exports = category_news_admin;
