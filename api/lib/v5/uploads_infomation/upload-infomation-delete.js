...
const function_export = function (id,res) {
	...
	var table_name  = config_database.PREFIX + "uploads_infomation ";
	var field_where  = config_database.PREFIX + "uploads_infomation_ID ";
	//create sql text
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: sql_text, timeout: 20000 }  , ( err , results , fields ) => {
				if( err ) {
					...
					return res.send({ 
						"error" : "10", 
						"position" : "lib->uploads_infomation->delete",
						"message": error_send 
					}); 
