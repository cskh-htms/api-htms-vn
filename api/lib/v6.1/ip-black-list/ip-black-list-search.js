...
const fields_get = require('./ip-black-list-fields-get.js');
...
const user_search = function (datas,res) {
	try{	
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗisearch, Vui lòng liên hệ admin" 
			);
		return res.send({ 
			"error" : "1",
			"position" : "lib/ip-black-list/user search",  
			"message": error_send 
			}); 
		...
		return new Promise( (resolve,reject) => {
			connection.query( { sql: get_sql_search_group, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					return ({ 
						"error" : "2",
						"position" : "lib/ip-black-list/ip-lacklist search", 
						"message": error_send 
					}); 
