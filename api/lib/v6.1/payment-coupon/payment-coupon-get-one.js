...
const fields_get = require('./payment-coupon-fields-get.js');
...
const function_export = function (id,res) {
	...
	try{	
		var get_sql_search_group = "SELECT " + 
			...
			"where " + config_database.PREFIX + "payment_coupon_ID =" + id;  
	}
	catch(error){
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗi lib->payment-coupon->get one, Vui lòng liên hệ admin" 
			);
		return res.send({ 
			"error" : "1",
			"position" : "lib->payment-coupon->get one", 
			"message": error_send 
			}); 
