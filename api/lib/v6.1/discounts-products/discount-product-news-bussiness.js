...
const discount_product_news_bussiness = async function (store_id,res) {
	...
	" where " + 
		config_api.PREFIX + "discount_program_product_link_status in (3) " +
		" and " + 
	...
	try {	
			...
			connection.query( { sql: sql_text, timeout: 20000 }, ( err , results , fields ) => {
				if( err ) {
					...
					var error_send = ojs_shares_show_errors.show_error( 
							evn, 
							err, 
							"Lỗi discont product news bussiness, Vui lòng liên hệ admin" 
						);
					return res.send({ 
						"error" : "1",
						"position" : "lib/discont product/discont product news bussiness", 
						"message": error_send 
					}); 
		...
		var error_send = ojs_shares_show_errors.show_error( 
				evn, 
				error, 
				"Lỗi discount dproduct news bussiness, Vui lòng liên hệ admin" 
			);
		return res.send({ 
			"error" : "2",
			"position" : "lib/discont product/discont product news bussiness",  
