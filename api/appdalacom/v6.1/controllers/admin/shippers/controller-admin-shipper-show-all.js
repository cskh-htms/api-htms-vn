...
async  function function_export(req, res, next) {
	...
	try{
		...
		try {
			...
			var shipper_id = 0;
			if(req.query.c1){
				shipper_id = req.query.c1;
			}else{
				return res.send({ 
					"error" : "01", 
					"position" : "api->appdalacom->controller->admin->shipper->show-all",
			...
			return res.send({ 
				"error" : "1", 
				"position" : "api->appdalacom->controller->admin->shipper->show-all-all",
				"message": error_send 
			}); 
				
		}	
		//return res.send([shipper_id]);
		//
		...
		//@
		//@ check phân quyền
		const check_owner_user_result = await check_owner_user(token,shipper_id,res);
		if(check_owner_user_result == "1" ){
			//go
		}
		else{
			var evn = ojs_configs.evn;
		...
		return res.send(["welcom !!! shipper"]);
