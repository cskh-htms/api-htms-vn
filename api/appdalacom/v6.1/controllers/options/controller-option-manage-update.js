...
const option_update = require('../../../../lib/' + config_api.API_LIB_VERSION + '/options/option-update');
	...
	try {	
		...
		try {
			...
			if(req.query.c1){
				...
				return res.send({ 
					"error" : "01", 
					"position" : "api->appdalacom->controller->options->manage->update",
		...
		//return res.send([option_id,datas]);
		//
		...
		if(check_role_result == "bussiness"){			
			const check_owner_store_resuilt = await check_owner_store(token,datas.options_product_speciality_stores_id,res);
		...
		//return res.send([check_role_result,"option_ok"]);
		//		
		...
		//@	
		//@ run database
		var result = await option_update(datas,option_id,res);
