...
//@
async  function controllers_discount_program_store_active(req, res, next) {
	...
	try {
		...
		res.send({ 
			"error" : "1", 
			"position" : "api/appdalacom/contriller/discounts/controllers-discount-program-store-active-admin",
			"message": error_send 
		}); 
		return;	
	}	
	...
	try{	
		...
		var data_discount = 		
		{
			...
			"order" :
			 [		 
				{    
					"field"  :"discount_program_details_date_created",
					"compare" : "DESC"
				}			
			]	
		...
		var fn_get_data_discount = new Promise((resolve, reject) => {
			let result = discount_detail_search(data_discount,res);
			resolve(result);
		});	
...
module.exports = controllers_discount_program_store_active;