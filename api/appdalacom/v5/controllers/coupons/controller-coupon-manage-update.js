	...
	try {	
		...
		try {
			...
			if(req.query.c1){
				...
				res.send({ 
					"error" : "01", 
					"position" : "api->appdalacom->controller->brands->manage->update",
		...
		if(check_role_result == "bussiness"){			
			const check_owner_store_resuilt = await check_owner_store(token,datas.coupon_speciality_stores_id_created,res);
			if(	check_owner_store_resuilt == "1" ){
				...
				res.send({ 
					"error" : "333",
					"position" : "api->appdalacom->controller->coupons->manage->update",
					"message": error_send 
				}); 
				return;			
			}				
		...
		//res.send([check_role_result,"coupon_ok"]);
		//return;		
		...
		//@	
		//@ run database
		var result = await coupon_update(datas,coupon_id,res);
