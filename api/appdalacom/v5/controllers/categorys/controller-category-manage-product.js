...
const category_get_one = require('../../../../lib/' + 
		config_api.API_LIB_VERSION + '/categorys/category-get-one');		
...
const product_search = require('../../../../lib/' + config_api.API_LIB_VERSION + '/products/product-search-by-category.js');
...
async  function store_order_get_all(req, res, next) {
	...
	try {
		...
		if(req.query.c1){
			...
			res.send({ 
				"error" : "01", 
				"position" : "api->appdalacom->controller->category->manage->product",
	...
	var category_by_store_resuilt = await category_get_one(category_id,res);
