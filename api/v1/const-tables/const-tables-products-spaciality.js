	...
	function get_select_fields(field_arr,sql_select_all){
		...
		if(Object.keys(field_arr).length == 0){
			...
			for (var x in field_arr){
				...
				//@
				//@
					sql_field_check  = "DATE_FORMAT(" + ojs_api_config.db_prefix  + field_arr[x] + "," + "'%Y/%m/%d %H:%i:%s'"  + ")";
				
				
				}else if(field_arr[x] == "products_speciality_price_sum" ){	
					sql_field_check  = "sum(" + ojs_api_config.db_prefix  + "products_speciality_price" + ")";				
					...
					sql_field_check  = " " + 			
						...
						"(CASE " + 
							"WHEN " +  
								ojs_api_config.db_prefix  + "products_speciality_sale_of_price IS NULL " + 
							"THEN " + 
								ojs_api_config.db_prefix  + "products_speciality_price " + 
								
								
							// date_star = null 	
							// date_end = null 
							"WHEN " +  
								ojs_api_config.db_prefix  + "products_speciality_date_start IS NULL and " + 
								ojs_api_config.db_prefix  + "products_speciality_date_end IS NULL " + 
							"THEN " + 
								ojs_api_config.db_prefix  + "products_speciality_sale_of_price " + 			
							...
							"WHEN " +  
								ojs_api_config.db_prefix  + "products_speciality_date_start IS NOT NULL and " + 
								ojs_api_config.db_prefix  + "products_speciality_date_end IS NULL and " + 
								"UNIX_TIMESTAMP(NOW()) - " + 
								"UNIX_TIMESTAMP(" + ojs_api_config.db_prefix  + "products_speciality_date_start ) > 0 " + 
							"THEN " + 
								ojs_api_config.db_prefix  + "products_speciality_sale_of_price " + 		
							...
							"WHEN " +  
								ojs_api_config.db_prefix  + "products_speciality_date_start IS NULL and " + 
								ojs_api_config.db_prefix  + "products_speciality_date_end IS NOT NULL and " + 
								"UNIX_TIMESTAMP(NOW()) - " + 
								"UNIX_TIMESTAMP(" + ojs_api_config.db_prefix  + "products_speciality_date_end ) < 0 " + 
							"THEN " + 
								ojs_api_config.db_prefix  + "products_speciality_sale_of_price " + 																	
							...
							"WHEN " +  
								...
								ojs_api_config.db_prefix  + "products_speciality_date_end IS NOT NULL and " + 
								"UNIX_TIMESTAMP(NOW()) - " + 
								"UNIX_TIMESTAMP(" + ojs_api_config.db_prefix  + "products_speciality_date_start ) > 0  and " + 
								"UNIX_TIMESTAMP(NOW()) - " + 
								"UNIX_TIMESTAMP(" + ojs_api_config.db_prefix  + "products_speciality_date_end ) < 0  " + 								
							"THEN " + 
								ojs_api_config.db_prefix  + "products_speciality_sale_of_price " + 			
					...
					sql_field_check = ojs_api_config.db_prefix + field_arr[x];
				}
	...
	if(Object.keys(order_arr).length == 0){
		...
		for (var x in order_arr){
			...
			if(	order_arr[x].field == "products_speciality_price_caution" 
				...
				sql_value_text  = ojs_api_config.db_prefix + order_arr[x].field;
			}
	...
	if(Object.keys(condition_arr).length == 0){
		...
		for (var x in condition_arr){
			for (var s in condition_arr[x].where){
				...
				//@@ edit date 
				if(
				ojs_functions_shares.check_date_full(condition_arr[x].where[s].value) == true 
				|| 
				ojs_functions_shares.check_date(condition_arr[x].where[s].value) == true 
				){
