...
//sql select default
let sql_select_all = 	"" + 	
	ojs_api_config.db_prefix  + "session_speciality_name as session_speciality_name, " + 
	ojs_api_config.db_prefix  + "session_speciality_line_order as session_speciality_line_order, " + 
	ojs_api_config.db_prefix  + "session_speciality_product_id as session_speciality_product_id, " + 
	ojs_api_config.db_prefix  + "session_speciality_qty as session_speciality_qty, " + 
	ojs_api_config.db_prefix  + "session_speciality_price as session_speciality_price, " + 
	ojs_api_config.db_prefix  + "session_speciality_discount as session_speciality_discount, " + 	
	ojs_api_config.db_prefix  + "session_speciality_unit_discount as session_speciality_unit_discount " 



//from table
let sql_from_default = 	" from " + 
	ojs_api_config.db_prefix + "session_speciality "  ;
...
let sql_link_search = 	""  + 
	...
	ojs_api_config.db_prefix + "session_speciality_product_id  = " + 
...
var insert_session_speciality = async function (datas) {
	//@
	let sql_text = "INSERT INTO " + ojs_api_config.db_prefix + "session_speciality  SET ?";
	let dataGo = {
...
var update_session_speciality = async function (datas,where) {
	...
	arrWhere.forEach(function(item) {
		...
		if(sqlWhere.length == 0){
			sqlWhere = ojs_api_config.db_prefix + item + '="' + mysql.escape(arrValueWhere[i]).replace(/^'|'$/gi, "") + '"';
		}else{
			sqlWhere = sqlWhere + ' and ' + ojs_api_config.db_prefix + item  + '= "' + mysql.escape(arrValueWhere[i]).replace(/^'|'$/gi, "")  + '"' ;
		}		
		i = i + 1 ;
	});	
	...
	let table_name  = ojs_api_config.db_prefix + "session_speciality ";
