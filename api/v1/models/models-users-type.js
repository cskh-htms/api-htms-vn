...
var insert_users_type = async function (datas) {
	//@
	let sql_text = "INSERT INTO " + ojs_api_config.db_prefix + "users_type  SET ?";
	let dataGo = {
...
var get_one_users_type = async function (users_type_id) {
	...
	let sql_text = 	"SELECT " +  sql_select_all + 
					...
					sql_link_default + 
					" and " + 
					ojs_api_config.db_prefix + "users_type_ID = '" + users_type_id + "' " + 
					sql_order_default
	//@
	try {
		return new Promise( (resolve,reject) => {
...
var update_users_type = async function (datas,users_type_id) {
	...
	let table_name  = ojs_api_config.db_prefix + "users_type ";
	let field_where  = ojs_api_config.db_prefix + "users_type_ID ";
	//create sql text
