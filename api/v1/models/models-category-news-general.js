...
let sql_select_all = 	"" + 	
	...
	ojs_api_config.db_prefix  + "category_news_parent_id as category_news_parent_id, " + 	
	
	"DATE_FORMAT(" + ojs_api_config.db_prefix  + "category_news_date_created," + "'%Y/%m/%d %H:%i:%s'"  + ") as category_news_date_created, " + 
	ojs_api_config.db_prefix  + "category_news_featured_image as category_news_featured_image, " + 
	ojs_api_config.db_prefix  + "category_news_infomation as category_news_infomation, " + 
	ojs_api_config.db_prefix  + "category_news_sort_order as category_news_sort_order, " + 
	ojs_api_config.db_prefix  + "category_news_show as category_news_show "

//from table
let sql_from_default = 	" from " + 
	ojs_api_config.db_prefix + "category_news "  ;
//link table	
let sql_link_default = 	"" ;
//link table	
let sql_order_default = " order by " + 
	ojs_api_config.db_prefix + "category_news_sort_order" ;
...
var insert_category_news_general = async function (datas) {
	//@
	let sql_text = "INSERT INTO " + ojs_api_config.db_prefix + "category_news  SET ?";
	let dataGo = {
...
var get_one_category_news_general = async function (category_news_id) {
	...
	let sql_text = 	"SELECT " +  sql_select_all + 
					...
					sql_link_default + 
					"where " + 
					ojs_api_config.db_prefix + "category_news_ID = '" + category_news_id + "' " + 
					sql_order_default
	//@
	try {
		return new Promise( (resolve,reject) => {
...
var update_category_news_general = async function (datas,category_news_id) {
	...
	let table_name  = ojs_api_config.db_prefix + "category_news ";
	let field_where  = ojs_api_config.db_prefix + "category_news_ID ";
	//create sql text
