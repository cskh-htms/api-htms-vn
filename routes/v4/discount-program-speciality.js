...
try {
	...
	//@
	//store_info
	var datas_send = ojs_datas_discount_program.get_store_info(store_id);
	...
	try {	
		...
		datas_info = {
			...
			'discount_program_list' : promise_result[3][8].datas,
			'discount_program_details_list' : promise_result[3][9].datas,
			'discount_program_product_list' : promise_result[3][10].datas,
		}	
...
//@
//@ 3 [/product-save]
router.post('asdasdasd/product-save/', async function(req, res, next) {
	//@
	//@
	//@
	//@
...
try {
	...
	//@ discount_program_tager
	var fn_discount_program_tager = new Promise((resolve, reject) => {
		var result = ojs_shares_fetch_data.get_data_send_token_get(
			ojs_configs.domain + '/api/' + ojs_configs.api_version + '/discount-program/' + discount_program_id,
			ojs_configs.token_supper_job
		);
		resolve(result);
	});	
	promise_all.push(fn_discount_program_tager);		
	//[4]
	...
	try {
		...
		datas_info = {
			...
			'discount_program_tager': promise_result[4].datas
		}
	...
	res.send( { "error" : "r_113" , "message" : error } );
}	
});
