...
router.post('/update/:order_id', async function(req, res, next) {
	...
	try {	
		...
		var active_update = await ojs_shares.get_data_send_token_put(ojs_configs.domain + '/api/' + check_datas_result.api_version + '/orders/speciality/' + order_id,datas, token);
...
router.post('/detail/update/:detail_id', async function(req, res, next) {
	...
	try {	
		//Lấy danh sách loại danh mục
		var active_update = await ojs_shares.get_data_send_token_put(ojs_configs.domain + '/api/' + check_datas_result.api_version + '/orders/speciality-detail/' + detail_id,datas, token);
		res.send(active_update);	
	}
	catch(error){
		var evn = ojs_configs.evn;
...
//
//
router.get('/detail/devare/:detail_id', async function(req, res, next) {
	//
	var token = req.session.token;	
	var detail_id = req.params.detail_id;
	//res.send([detail_id]);
	...
	try{
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, error, "server đang bận, truy cập lại sau" );
		res.send({ "error" : "1.router_app->orders-speciality->detail->devare", "message": error_send } ); 
		return;			
	}
	...
	if(check_datas_result.error != "" ){
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, "Không đủ quyền truy cập dữ liệu", "Không đủ quyền truy cập dữ liệu" );
		res.send({ "error" : "2.router_app->orders-speciality->detail->devare", "message": error_send } ); 
		return;			
	}	
	...
	if(check_datas_result.user_role != "admin" ){
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, "Không đủ quyền truy cập dữ liệu", "Không đủ quyền truy cập dữ liệu" );
		res.send({ "error" : "3.router_app->orders-speciality->detail->devare", "message": error_send } ); 
		return;			
	}		
	//send web
	//@sidebar_type -- loại sibar 
	...
	try {	
		//Lấy danh sách loại danh mục
		var active_devare = await ojs_shares.get_data_send_token_devare(ojs_configs.domain + '/api/' + check_datas_result.api_version + '/orders/speciality-detail/' + detail_id, token);
...
//
//
router.get('/devare/:order_id', async function(req, res, next) {
	//
	var token = req.session.token;	
	var order_id = req.params.order_id;
	//res.send([order_id]);
	...
	try{
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, error, "server đang bận, truy cập lại sau" );
		res.send({ "error" : "1.router_app->orders-speciality->->devare", "message": error_send } ); 
		return;			
	}
	...
	if(check_datas_result.error != "" ){
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, "Không đủ quyền truy cập dữ liệu", "Không đủ quyền truy cập dữ liệu" );
		res.send({ "error" : "2.router_app->orders-speciality->->devare", "message": error_send } ); 
		return;			
	}	
	...
	if(check_datas_result.user_role != "admin" ){
		...
		////evn = "dev";;
		var error_send = ojs_shares.show_error( evn, "Không đủ quyền truy cập dữ liệu", "Không đủ quyền truy cập dữ liệu" );
		res.send({ "error" : "3.router_app->orders-speciality->->devare", "message": error_send } ); 
		return;			
	}		
	//send web
	//@sidebar_type -- loại sibar 
	...
	try {	
		//Lấy danh sách loại danh mục
		var active_devare = await ojs_shares.get_data_send_token_devare(ojs_configs.domain + '/api/' + check_datas_result.api_version + '/orders/speciality/' + order_id, token);
		res.send(active_devare);	
	}
	catch(error){
		var evn = ojs_configs.evn;
		...
		res.send({ "error" : "33.router_app->orders-speciality->->devare", "message": error_send } ); 
		return;		
	}		
});
...
router.post('/save_fee', async function(req, res, next) {
	...
	try {	
		...
		var active_save = await ojs_shares.get_data_send_token_post(ojs_configs.domain + '/api/' + check_datas_result.api_version + '/orders/speciality-detail/',datas, token);
